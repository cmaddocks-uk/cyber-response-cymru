...
**Tool:** Cyber Incident Response Planner — UK Schools & Colleges
**Live site:** [cmaddocks-uk.github.io/cyber-response](https://cmaddocks-uk.github.io/cyber-response)
**Version covered:** v1.7.0 and later
...
- GitHub issue (if non-sensitive): [github.com/cmaddocks-uk/cyber-response/issues](https://github.com/cmaddocks-uk/cyber-response/issues)
...
The tool is a **single static HTML file** hosted on GitHub Pages. There is no server, no database, no user accounts, no remote API beyond a single privacy-friendly analytics beacon. Plan data is held in the user's browser session only and never leaves their device unless they explicitly export it as a JSON file.

Below is what the tool actively defends against, what it doesn't claim to defend against, and what is out of scope.

### In scope — actively mitigated

| Threat | Mitigation |
|---|---|
| **Cross-site scripting (XSS)** via plan content | All user input HTML-escaped before rendering. Strict Content Security Policy: `default-src 'none'; script-src 'self' 'unsafe-inline' https://gc.zgo.at; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com; img-src 'self' data: https://cyber-response.goatcounter.com; connect-src https://cyber-response.goatcounter.com; form-action 'none'; frame-ancestors 'none'; base-uri 'none';` |
...
| **Domain abuse / fake URLs** (e.g. `cmaddocks-uk.github.io/cyber-response/powerautomate/...`) | See the dedicated section below. |
...
- **Phishing emails referring to the tool.** A phishing email could plausibly link to the real tool — that's not something this site can prevent. The tool itself only loads from `cmaddocks-uk.github.io/cyber-response`; verify the URL.
...
```
cmaddocks-uk.github.io/cyber-response/powerautomate/your-flow-here
cmaddocks-uk.github.io/cyber-response/api/auth/microsoft
cmaddocks-uk.github.io/cyber-response/webhook/triggers/...
```
...
5. **GoatCounter analytics** records hits per path. Suspicious traffic patterns to abuse paths can be detected after the fact.

### What this site cannot prevent

- Someone copying or screenshotting a fake URL and sending it to victims.
- A third party referencing the fake URL on social media or in phishing campaigns.
- Search-engine snapshots that pre-date the `robots.txt` rules.

### What to do if you receive a suspicious URL using this domain

1. **Do not click it.** The real tool is at `cmaddocks-uk.github.io/cyber-response` with **no further path components**.
...
4. Notify the author via the issue route above so the 404 page and robots.txt can be hardened against the specific path being abused.
...
`security-test.js` is a Node.js test suite covering:

- XSS injection in plan fields
- Prototype pollution via JSON import
- Schema validation rejection of malformed JSON
- Link-hardening verification (`rel="noopener noreferrer"` present on all `target="_blank"` anchors)

Run with `node security-test.js` from the repo root.

---

## Last reviewed

This document is reviewed at each minor version bump. Last reviewed: **v1.7.0 (10 May 2026).**
